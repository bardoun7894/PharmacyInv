...
Public Class frmSoldItem
    ...
    Sub GetSoldTop()
        Dim i As Integer = 0
        Try
            If cboTop.Text = String.Empty Then Return
    ...
    Sub GetSoldType()
        ...
        Try
            ...
            While dr.Read()
                i += 1
                dataGridView2.Rows.Add(i, dr.Item("pid").ToString, dr.Item("brand").ToString,
               dr.Item("generic").ToString, dr.Item("classification").ToString, dr.Item("formulation").ToString,
               dr.Item("type").ToString, dr.Item("sum(ca.qty)").ToString)
            End While
            cn.Close()
            dr.Close()
        Catch ex As Exception
    ...
    Private Sub cboTop_SelectedValueChanged(sender As Object, e As EventArgs) Handles cboTop.SelectedValueChanged
        GetSoldTop()
    End Sub

    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If cboTop.Text IsNot String.Empty Then
            GetSoldTop()
        Else
            GetSoldType()
        End If
